...
while True:
    ...
    print('---------------------------------------')
    print('Klienti u lidh me hostadresen %s ne portin %s' %address +'.')

    OperacioniIArdhurNgaKlienti = clientS.recv(128)   # Ketu vjen operacioni nga klienti.
    operacioni = str(OperacioniIArdhurNgaKlienti.decode("utf-8"))   # Operacioni nuk vjen si string andaj e dekodojme ne utf-8.
    print("Klienti ka zgjedhur operacionin: " + operacioni)
    ...
    if operacioni=="IPADDRESS":   # Metoda IPADDRESS
        ...
        clientS.send(textEncoded)                     # andaj e kthejme ne byte.
        clientS.close()   # Ketu e mbyllim.
        ...
        print('Kalkulimi nga serveri: Porti eshte: ' + str(address[1]) +'.')   # Portin e nxjerrim nga vektori Address, elementin e dyte te tije.

        text = str(address[1]) + '.'

        textEncoded = text.encode(encoding='UTF-8')

        clientS.send(textEncoded)
        clientS.close()
        print('Perfundoi komunikimi.')
        ...
        KerkesaEKlientit = clientS.recv(128)
        short = str(KerkesaEKlientit.decode('utf-8'))
        ...
        a = str(zanore) + ' zanore dhe ' + str(bashketingellore) + ' bashketingellore.'
        ae = a.encode(encoding='utf-8')

        print('Kerkesa nga klienti: Sa zanore dhe bashketingellore ka teksti: ' + short + '.')
        ...
        KerkesaEKlientit = clientS.recv(128)
        str1 = str(KerkesaEKlientit.decode("utf-8"))
        ...
        print('Kerkesa nga klienti: A eshte ky tekst polindrom apo jo: ' + str(KerkesaEKlientit.decode("utf-8"))+'.')

        rev = ''.join(reversed(str(KerkesaEKlientit.decode("utf-8"))))   # Kam perdorur nje funksion te predefinuar ne python
                                                                         # ''.join(reversed(s)) qe e kthen mbrashte textin.

        if (str(KerkesaEKlientit.decode("utf-8")) == rev):   # Nese teksti i kthyer mbrashte eshte i barabarte me textin fillestar                                 # atehere ai tekst eshte polindrom.
            text1 = 'eshte'
        else:
            text1 = 'nuk eshte'
        ...
        KerkesaEKlientit = clientS.recv(128)
        print('Kerkesa nga klienti: ' + str(KerkesaEKlientit.decode("utf-8")))

        def Convert(string):   # Ketu nga klienti e marrim si p.sh. gcf 10 20, kete tekst(string), e kthejme ne varg.
            li = string.split(" ") 
            return li 

        list = Convert(str(KerkesaEKlientit.decode("utf-8")))   # Listen e mbushim ketu.
        ...
        # Sqarim: Kam mundur te beje kete detyre ne menyre me te thjeshte, por keshtu eshte kerkuar.
        ...
        virusi = Convert(str(KerkesaEKlientit.decode("utf-8")))
